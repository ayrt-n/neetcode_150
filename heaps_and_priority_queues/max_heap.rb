...
class MaxHeap
  ...
  def heapify_down
    ...
    while left_child?(c_ind)
      larger_index = left_child_index(c_ind)
      larger_index = right_child_index(c_ind) if right_child?(c_ind) && right_child(c_ind) > left_child(c_ind)
      return if curr > @heap[larger_index]

      swap(c_ind, larger_index)
      c_ind = larger_index
      curr = @heap[c_ind]
    end
  end
